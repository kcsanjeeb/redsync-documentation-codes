# src/train_freeze_phase2_async_fixed_fc_naive_v2.py
import argparse
import json
import os
import queue
...
class FreezeController:
    ...
    Plasticity can be absolute or relative (normalized by ref magnitude).
    """
    def __init__(
        self,
        W: int,
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # PROBE + (optional) naive FeatureCache overhead
        if epoch >= args.bootstrap_epochs and args.probe_every_epochs > 0 and epoch % args.probe_every_epochs == 0:
            ...
            with torch.no_grad():
                ...
                # ---- Naive cache path: capture + load every probe after freeze_k threshold ----
                frozen_k = frozen_upto + 1
                do_fc = (fc is not None) and (frozen_k >= args.fc_enable_after_freeze_k)

                if do_fc:
                    key = f"probe_b{next_idx}"
                    # capture (miss path is included), then immediately load to force overhead visibility
                    cap_log = fc.capture(key, act.to("cpu", dtype=torch.float32).contiguous())
                    load_log = fc.load(key, device=device)
                    act2 = load_log.pop("act")  # tensor
                    cache_log = {
                        "fc_enabled": True,
                        "fc_key": key,
                    ...
                    act_to_send = act2.to("cpu", dtype=torch.float32).contiguous()
                else:
