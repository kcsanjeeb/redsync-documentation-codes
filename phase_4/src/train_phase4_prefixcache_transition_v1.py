# src/train_phase4_prefixcache_transition_v1.py
...
def build_feature_cache(
    ...
    loader: DataLoader,
    device: torch.device,
    out_dtype: torch.dtype = torch.float32,  # default float32 for correctness
...
def main():
    ...
    # NEW: suffix optimizer LR after transition
    ap.add_argument(
        "--suffix_lr",
        type=float,
        default=0.01,
        help="Learning rate for suffix optimizer after transition",
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (not transitioned) and (epoch >= args.transition_epoch):
            ...
            feats, labels, cache_log = build_feature_cache(
                prefix, cache_build_loader, device, out_dtype=torch.float32
            )
        ...
        print(
            ...
            f"| time={ep_s:.2f}s | transitioned={transitioned}"
        )
